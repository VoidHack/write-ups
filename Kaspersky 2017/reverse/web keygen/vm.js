...
class UniRegHandler {
    constructor(vm) {
        this.registers = vm.registers;
        this.index = vm.getCurrentByte();
        this.uniteSize = vm.getCurrentByte();
        this.coeff = vm.getCurrentByte();   // ??????????
    }

    getValue() {
        switch( this.uniteSize ) {
            case 1:
                return this.registers.getUint8(this.index)*this.coeff;
            case 2:
                return this.registers.getUint16(this.index)*this.coeff;
            case 4:
                return this.registers.getUint32(this.index)*this.coeff;
        }
    }
    ...
    setValue(value) {
        switch( this.uniteSize ) {
            case 1:
                this.registers.setUint8(this.index, value);
                break;
            case 2:
                this.registers.setUint16(this.index, value);
                break;
            case 4:
                this.registers.setUint32(this.index, value);
                break;
        }
        this.coeff = 1;
    }
...
class ConstantHandler // get constant from text section
{
    constructor(vm)
    {
        this.value = vm.getMemValU32();
    }
...
class ParentRegModifier {
    constructor(vm) {
        this.address = 0;

        // SOME BLACK MAGIC
        var iter = vm.getCurrentByte();
        while( iter-- ) {
            this.address += vm.getHandler().getValue();
            PrintVM(vm)
        }

        this.address &= 0xFFFFFFFF; // if value is greater than 0xFFFFFFFF, then we cut it.
...
class AddressHandler8 extends ParentRegModifier  {
    getValue() {
        return this.stack.getUint8(this.address);
    }

    setValue(a20) {
        console.log('Handler: ' + this.address.toString(16));
        this.stack.setUint8(this.address, a20);
    }
}

class AddressHandler32 extends ParentRegModifier {
    getValue() {
        return this.stack.getUint32(this.address);
    }

    setValue(value) {
        console.log('Handler: ' + this.address.toString(16));
        this.stack.setUint32(this.address, value);
    }
}
...
class VirtualMachine
{
    constructor(memory)
    {
        this.stack = new DataView(memory.buffer);
        this.IP = 0;
        this.registers = new DataView(new ArrayBuffer(32));
        var SP = memory.length;
        this.registers.setInt32(16,SP);
    ...
    getMemValU32() //для констант
    {
        var value = this.stack.getUint32(this.IP);
        this.IP += 4;
        return value;
    }
    ...
    getCurrentByte()
    {
        var instruction = this.stack.getUint8(this.IP);
        this.IP++;
        return instruction;
    }
    ...
    getHandler()
    {
        var handlerType = this.getCurrentByte();
        switch(handlerType )
        {
            case 1:
                return new UniRegHandler(this);
            case 2:
                return new ConstantHandler(this);
            case 3:
                return new AddressHandler8(this);
            case 4:
                return new AddressHandler32(this);
        }
        return null;
    }
    ...
    pushInt32(value)
    {
        var SP = this.registers.getInt32(16) - 4;
        this.registers.setInt32(16,SP);
        this.stack.setUint32(SP,value)
        return SP;
    }

    popInt32()
    {
        var SP = this.registers.getInt32(16);
        var stackValue = this.stack.getUint32(SP);
        SP += 4;
        this.registers.setInt32(16,SP);
        return stackValue;
    }
    ...
    printStack(num) {
        var MEMLEN = 11350 - 4
        for (var i = 0; i <= Math.min(num, MEMLEN - this.registers.getInt32(16)); i += 4) {
            console.log((this.registers.getInt32(16) + i).toString() + " : " + this.stack.getInt32(this.registers.getInt32(16) + i).toString());
        }
    }
    ...
    execute()
    {
        var instruction = this.getCurrentByte();
         switch(instruction)
        {
            case 1: // push h
                var handler = this.getHandler();
                printHandler(handler, 1);
                this.pushInt32(handler.getValue());
                break;
            case 2: // mov, h1, h2
                var handler1 = this.getHandler();
                var handler2 = this.getHandler();

                handler1.setValue(handler2.getValue());

                printHandler(handler1, 1);
                // printHandler(handler2, 2);
                break;
            case 3: // sub h1, h2
                var handler1 = this.getHandler();
                var handler2 = this.getHandler();

                var sub = handler1.getValue() + ((~handler2.getValue()) + 1);
                this.isNegative = new Boolean(sub>>32);
                handler1.setValue(sub);

                printHandler(handler1, 1);
                break;
            case 4: // call h
                var handler = this.getHandler();
                var handlerValue = handler.getValue();

                this.pushInt32(this.IP);
                this.IP = handlerValue;
                break;
            case 5: // add h1, h2
                var handler1 = this.getHandler();
                var handler2 = this.getHandler();

                var sum = handler1.getValue() + handler2.getValue();

                this.isNegative = new Boolean(sum>>32);
                handler1.setValue(sum);
            ...
            case 6: // cmp h1, h2
                ...
                this.isEqual = (handler1.getValue() == handler2.getValue());

                var sum = handler1.getValue() + ((~handler2.getValue()) + 1);
                this.isNegative = new Boolean(sum>>32);
                break;
            case 7: // jne h
                var handler = this.getHandler();
            ...
            case 10: // movz h1, h2
                ...
                handler1.setValue(handler2.getValue()>>>0);
            ...
            case 11: // xor h1, h2
                ...
                var xor = handler1.getValue() ^ handler2.getValue();
                handler1.setValue(xor);

                printHandler(handler1, 1);
                break;
            case 12: // magic instruction
                var handler = this.getHandler();

                var a20 = (this.registers.getInt32(8) << 32) | this.registers.getInt32(0);
                var a44 = a20 / handler.getValue();
                this.registers.setInt32(0,a44);
                var a44 = a20 % handler.getValue();
                this.registers.setInt32(8,a44);
                break;
            case 13: // or h1, h2
                var handler1 = this.getHandler();
                var handler2 = this.getHandler();

                var or = handler1.getValue() | handler2.getValue();
                handler1.setValue(or);
            ...
            case 17: // and h1, h2
                ...
                var and = handler1.getValue() & handler2.getValue();
                handler1.setValue(and);
...
function GetFlag(a66)
{
    ...
    console.log(vm.registers.getInt32(0));

    if ( vm.registers.getInt32(0) >= 0 )
        return new TextDecoder("utf-8").decode(memory.subarray(modified_SP_1, modified_SP_1 + 37));
...
function PrintVM(vm) {
    ...
    res = ""
    res += "-----------Memory:-----------\n";
    for (var i=-3; i < Math.min(5, MEMLEN - i - vm.IP ); ++i)
    {   if (i + vm.IP < 0)
            continue;
        if (i == 0) {
            res += ("=> " + (i + vm.IP).toString() + ' : ' + vm.stack.getUint8(i + vm.IP).toString() + "\n");
            continue
        }
        if (i + vm.IP < 11350)
            res += ("   " + (i + vm.IP).toString() + ' : ' + vm.stack.getUint8(i+vm.IP).toString() + "\n")
    ...
    res += "----------Stack:----------\n";
    for (var i = 0; i <= Math.min(76, MEMLEN - vm.registers.getInt32(16)); i += 4) {
        res += ((vm.registers.getInt32(16) + i).toString() + " : " + vm.stack.getInt32(vm.registers.getInt32(16) + i).toString() + "\n");
    }
    res += "----------Registers:----------\n";
    for (var i=0; i < 32; i += 4)
        res += ((i).toString() + ' : ' + vm.registers.getInt32(i).toString() + "\n");
    res += ("------------------------------\n")
...
function printHandler(h, i) {
    console.log("handler_" + (i).toString() + ": " + h.getValue().toString())
}


console.log(GetFlag('8XcCDABD'));
